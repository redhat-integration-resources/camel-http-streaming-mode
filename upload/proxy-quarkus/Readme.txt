...
This project contains logic to proxy an HTTP data stream tp the server system. The aim is to test Camel's ability to handle large HTTP data streams.
...
1. Make sure the server (server-quarkus) is running.
...
3. Run the JAR file with low memory settings:
	...
	java -jar -Xms200m -Xmx200m target/quarkus-app/quarkus-run.jar


4. From the client folder send a request using a large data stream.
   IMPORTANT: use port 9000 to send the request.


5. Stop the engine and inspect the result file.

  Camel Quarkus should proxy the HTTP byte stream to the server which dumps it in a file in the 'data' directory.
